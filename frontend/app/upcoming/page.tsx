...
export default function Home() {
    ...
    async function fetchFeatured() {
        ...
        nextYear.setFullYear(today.getFullYear() + 1, today.getMonth(), today.getDate())
    ...
    function getTimeUntilMovie(release_date: string): ReactNode
    {
        ...
        let difference = Math.floor((release.getTime() - Date.now())/(1000 * 60 * 60 * 24)); 
        if (difference < 0)
            difference = 0;
        ...
        return (
            <div className={`badge align-center ${badgeClass}`}>
                <p>{display}</p>
            </div>
        );
    }
            ...
            {loading ? (
                ...
                <div className="flex flex-wrap gap-5 py-4 justify-items-center">
                    {featured.map((movie: Movie, index: number) => (
                        <div className='flex flex-col items-center gap-2' key={movie.id}>
                            <MovieModal 
                                key={movie.id} 
                                movie={movie}
                                isOpen={isOpen[index]} 
